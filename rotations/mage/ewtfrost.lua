...
if not player.moving and  talent(7,3) and castable(SB.GlacialSpike, 'target') and -buff(SB.BrainFreeze) and -spell(SB.GlacialSpike) == 0 then 
  return cast(SB.GlacialSpike) 
   ...
   -- WHhen cast at ground 'target' works, put this back in
    ...
    if   -buff(SB.WintersReach) and -spell(SB.Flurry) == 0 and not player.spell(SB.Flurry).lastcast then
      return cast(SB.Flurry)
    end

    if   -buff(SB.WintersReachOther) and -spell(SB.Flurry) == 0 and not player.spell(SB.Flurry).lastcast then
      return cast(SB.Flurry)
    end
    ...
    if modifier.shift and -spell(SB.Blizzard) == 0 then
      cast(SB.Blizzard, 'target')
      ClickPosition(X,Y,Z)
    end

    if talent(7,3) and castable(SB.GlacialSpike, 'target') and toggle('gspike', false) then 
      return cast(SB.GlacialSpike)
    end

    if castable(SB.Ebonbolt, 'target') and toggle('cooldowns', false) and not -buff(SB.BrainFreeze) and buff(SB.Icicles).count >= 4 then 
...
dark_addon.rotation.register({
  ...
  name = 'ewtfrost',
  label = 'EWT Compatible Frost by Rotations',
  combat = combat,
  gcd = gcd,
  resting = resting,
  interface = interface
